...
async def _persist_messages(
    ...
    try:
        ...
        logger.info(
            "METRIC stream_persist user=%s session=%s total_ms=%.2f chars=%d reasoning=%s",
            user_id,
            session_id,
        ...
        logger.exception("failed to persist stream messages")
...
async def stream_chat(
    ...
    if not await check_session_owned_cached(request.sessionId, current_user, supabase):
